...
NAGIOS_COMMAND_MAPPING = {
    ...
    # service commands
    ('service', 'acknowledge'): ['ACKNOWLEDGE_SVC_PROBLEM;{host_name};{service_desc};1;1;0;telegram-bot;Acknowledged by telegram'],
    ('service', 'downtime') : ['SCHEDULE_SVC_DOWNTIME;{host_name};{service_desc};{start_time};{end_time};1;0;{downtime_duration};telegram-bot;Scheduled by telegram']
}
...
def get_action_keyboard(bot, update):
    ...
    for action, key_text in SUPPORTED_ACTIONS.items():
        ...
        keyboard_options.append(InlineKeyboardButton(key_text, callback_data=callback_data))
...
def process_menu_option(bot, update):
    ...
    if callback_data in ('nagios action downtime', 'nagios action acknowledge'):
        ...
        update.callback_query.edit_message_text("Please send the alert that you would like to action")
        return
